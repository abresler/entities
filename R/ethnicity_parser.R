.classify_last_name <- function(last_name = "Smith", return_message = T, include_probabilities = F) {
  library(wru)
  if (return_message) {
  ...
  df <-
    ...
    mutate(race =  race %>% str_remove_all("pred."),
           race =   case_when(race == "asi" ~ "Asian",
                              race == "bla" ~ "Black",
                              race == "his" ~ "Hispanic",
                              race == "whi" ~ "White",
                              TRUE ~ "Other"))
...
classify_last_names <-
  function(last_names = c("Jackson", "Cheng", "Wen"), include_probabilities = F, last_name_column = "nameLast", return_message = T) {
  ...
  all_data <-
    ...
    map_dfr(function(last_name){
      .classify_last_name_safe(last_name = last_name, return_message = return_message, include_probabilities = include_probabilities)
    })
...
tbl_last_name <-
  ...
  remove_cols <- names(data)[names(data) %in% names(df_names)] %>% str_remove_all(name_column) %>% discard(function(x){
    x == ""
  })
