  ...
  function() {
    words <-
      c(" & ", " ag ", " and", " as ", " at ", " authority ", " bank ",
        ...
        "immobiliare", "inc", "incorp", "incorporated", "individually",
...
.parse_party_data <-
  function(party = "LIU, YUAN-CHI",
    ...
    if (is_entity) {
      party_clean <-
        party_original %>% str_remove_all("\\,|\\.") %>% gsub("\\s+", " ", .)
