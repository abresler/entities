...
.parse_region_json <-
  function(url = "https://forebears.io/assets/geoJSON/17092.Regions.json") {
    ...
    idCountry <-
      parts[[length(parts)]] %>% str_split("\\.") %>% flatten_chr() %>%  .[[1]] %>% as.numeric()
...
.parse_fb_name_url <-
  function(url = "https://forebears.io/a?c=Name&m=getGeo&i=209524&p=18000", return_wide = T) {
...
.parse_fb_surname_url <-
  function(url = "https://forebears.io/surnames/zhao",
    ...
    .parse_table_country_safe <-
      possibly(.parse_table_country, tibble())
    .parse_table_similar_safe <-
      possibly(.parse_table_similar, tibble())
    ...
    data <-
      tibble(
        rankNameWorld = world_rank,
        countPopulationNameWorld = world_pop,
        descriptionName = meaning,
        urlForebearsName = url
      )
...
us_political_affiliation <-
  memoise::memoise(function(snake_names = F) {
    ...
    countryRank <-
      page %>% html_nodes("td:nth-child(5)") %>% map_chr(function(x) {
...
fb_locations <-
  function(locations = NULL,
    ...
    .parse_country_url_safe <-
      possibly(.parse_country_url, tibble())
...
download_fb_names <-
  function(last_names = c(),
    ...
    missing_ln <-
      df_ln %>%
      left_join(df_finished, by = "slug_ln") %>%
